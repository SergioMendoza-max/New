...
public class GhostControllers {
    ...
    public void deleteCharacterById(@PathVariable Long id){ghost.deleteCharacterById(id);}
    ...
    public String getBio(@PathVariable String name){return ghost.getBio(name);}

    @RequestMapping(value="/updating/{id}",method=RequestMethod.POST)
    public GhostDto updateCharacter(@RequestBody GhostDto ghostDto, @PathVariable Long id){return ghost.updateCharacter(id,ghostDto);}

    @RequestMapping(value="/character/{name}", method=RequestMethod.GET)
    public GhostDto getByName(@PathVariable String name){return ghost.getByName(name);}
}
